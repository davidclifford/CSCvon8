EESchema Schematic File Version 4
EELAYER 30 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 4 6
Title ""
Date ""
Rev ""
Comp ""
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
Text Notes 6050 5400 0    60   ~ 0
Jump\nLogic
$Comp
L power:GNDREF #PWR014
U 1 1 5C911F8E
P 6950 6050
F 0 "#PWR014" H 6950 5800 50  0001 C CNN
F 1 "GNDREF" H 6950 5900 50  0000 C CNN
F 2 "" H 6950 6050 50  0001 C CNN
F 3 "" H 6950 6050 50  0001 C CNN
	1    6950 6050
	1    0    0    -1  
$EndComp
$Comp
L power:VCC #PWR015
U 1 1 5C911FB4
P 6950 5850
F 0 "#PWR015" H 6950 5700 50  0001 C CNN
F 1 "VCC" H 6950 6000 50  0000 C CNN
F 2 "" H 6950 5850 50  0001 C CNN
F 3 "" H 6950 5850 50  0001 C CNN
	1    6950 5850
	1    0    0    -1  
$EndComp
$Comp
L Schematic-rescue:74LS574-RESCUE-Schematic-74LS593 IC7
U 1 1 5C98EE54
P 3000 1750
F 0 "IC7" H 3000 2450 50  0000 C CNN
F 1 "74HCT574" H 3000 2350 50  0000 C CNN
F 2 "Housings_DIP:DIP-20_W7.62mm_Socket" H 3000 1750 50  0001 C CNN
F 3 "" H 3000 1750 50  0001 C CNN
	1    3000 1750
	1    0    0    -1  
$EndComp
$Comp
L Schematic-rescue:74LS574-RESCUE-Schematic-74LS593 IC8
U 1 1 5C98FD7B
P 2950 3700
F 0 "IC8" H 2950 4400 50  0000 C CNN
F 1 "74HCT574" H 2950 4300 50  0000 C CNN
F 2 "Housings_DIP:DIP-20_W7.62mm_Socket" H 2950 3700 50  0001 C CNN
F 3 "" H 2950 3700 50  0001 C CNN
	1    2950 3700
	1    0    0    -1  
$EndComp
Entry Wire Line
	3900 3900 4000 3800
Entry Wire Line
	3900 3800 4000 3700
Entry Wire Line
	3900 3700 4000 3600
Entry Wire Line
	3900 3600 4000 3500
Entry Wire Line
	3900 3500 4000 3400
Entry Wire Line
	3900 3400 4000 3300
Entry Wire Line
	3900 3300 4000 3200
Entry Wire Line
	3900 3200 4000 3100
Wire Wire Line
	3650 3200 3900 3200
Wire Wire Line
	3650 3300 3900 3300
Wire Wire Line
	3650 3400 3900 3400
Wire Wire Line
	3650 3500 3900 3500
Wire Wire Line
	3650 3600 3900 3600
Wire Wire Line
	3650 3700 3900 3700
Wire Wire Line
	3650 3800 3900 3800
Wire Wire Line
	3650 3900 3900 3900
Text Label 3700 3900 0    60   ~ 0
adr0
Text Label 3700 3800 0    60   ~ 0
adr1
Text Label 3700 3700 0    60   ~ 0
adr2
Text Label 3700 3600 0    60   ~ 0
adr3
Text Label 3700 3500 0    60   ~ 0
adr4
Text Label 3700 3400 0    60   ~ 0
adr5
Text Label 3700 3300 0    60   ~ 0
adr6
Text Label 3700 3200 0    60   ~ 0
adr7
Entry Wire Line
	3900 1250 4000 1150
Entry Wire Line
	3900 1350 4000 1250
Entry Wire Line
	3900 1450 4000 1350
Entry Wire Line
	3900 1550 4000 1450
Entry Wire Line
	3900 1650 4000 1550
Entry Wire Line
	3900 1750 4000 1650
Entry Wire Line
	3900 1850 4000 1750
Entry Wire Line
	3900 1950 4000 1850
Wire Wire Line
	3700 1250 3900 1250
Wire Wire Line
	3700 1350 3900 1350
Wire Wire Line
	3700 1450 3900 1450
Wire Wire Line
	3700 1550 3900 1550
Wire Wire Line
	3700 1650 3900 1650
Wire Wire Line
	3700 1750 3900 1750
Wire Wire Line
	3700 1850 3900 1850
Wire Wire Line
	3700 1950 3900 1950
Text Label 3700 1950 0    60   ~ 0
adr8
Text Label 3700 1850 0    60   ~ 0
adr9
Text Label 3700 1750 0    60   ~ 0
adr10
Text Label 3700 1650 0    60   ~ 0
adr11
Text Label 3700 1550 0    60   ~ 0
adr12
Text Label 3700 1450 0    60   ~ 0
adr13
Text Label 3700 1350 0    60   ~ 0
adr14
Text Label 3700 1250 0    60   ~ 0
adr15
Text Notes 2900 1650 0    60   ~ 0
ARhi
Text Notes 2850 3600 0    60   ~ 0
ARlo
Entry Wire Line
	1950 1350 2050 1250
Entry Wire Line
	1950 1450 2050 1350
Entry Wire Line
	1950 1550 2050 1450
Entry Wire Line
	1950 1650 2050 1550
Entry Wire Line
	1950 1750 2050 1650
Entry Wire Line
	1950 1850 2050 1750
Entry Wire Line
	1950 1950 2050 1850
Entry Wire Line
	1950 2050 2050 1950
Wire Bus Line
	1950 4900 1200 4900
Entry Wire Line
	1950 3300 2050 3200
Entry Wire Line
	1950 3400 2050 3300
Entry Wire Line
	1950 3500 2050 3400
Entry Wire Line
	1950 3600 2050 3500
Entry Wire Line
	1950 3700 2050 3600
Entry Wire Line
	1950 3800 2050 3700
Entry Wire Line
	1950 3900 2050 3800
Entry Wire Line
	1950 4000 2050 3900
Wire Wire Line
	2050 3200 2250 3200
Wire Wire Line
	2050 3300 2250 3300
Wire Wire Line
	2050 3400 2250 3400
Wire Wire Line
	2050 3500 2250 3500
Wire Wire Line
	2050 3600 2250 3600
Wire Wire Line
	2050 3700 2250 3700
Wire Wire Line
	2050 3800 2250 3800
Wire Wire Line
	2050 3900 2250 3900
Text Label 2200 3900 0    60   ~ 0
d0
Text Label 2200 3800 0    60   ~ 0
d1
Text Label 2200 3700 0    60   ~ 0
d2
Text Label 2200 3600 0    60   ~ 0
d3
Text Label 2200 3500 0    60   ~ 0
d4
Text Label 2200 3400 0    60   ~ 0
d5
Text Label 2200 3300 0    60   ~ 0
d6
Text Label 2200 3200 0    60   ~ 0
d7
Wire Wire Line
	2050 1250 2300 1250
Wire Wire Line
	2050 1350 2300 1350
Wire Wire Line
	2050 1450 2300 1450
Wire Wire Line
	2050 1550 2300 1550
Wire Wire Line
	2050 1650 2300 1650
Wire Wire Line
	2050 1750 2300 1750
Wire Wire Line
	2050 1850 2300 1850
Wire Wire Line
	2050 1950 2300 1950
Text Label 2100 1950 0    60   ~ 0
d0
Text Label 2100 1850 0    60   ~ 0
d1
Text Label 2100 1750 0    60   ~ 0
d2
Text Label 2100 1650 0    60   ~ 0
d3
Text Label 2100 1550 0    60   ~ 0
d4
Text Label 2100 1450 0    60   ~ 0
d5
Text Label 2100 1350 0    60   ~ 0
d6
Text Label 2100 1250 0    60   ~ 0
d7
$Comp
L power:VCC #PWR016
U 1 1 5C99A807
P 3850 2150
F 0 "#PWR016" H 3850 2000 50  0001 C CNN
F 1 "VCC" H 3850 2300 50  0000 C CNN
F 2 "" H 3850 2150 50  0001 C CNN
F 3 "" H 3850 2150 50  0001 C CNN
	1    3850 2150
	1    0    0    -1  
$EndComp
$Comp
L power:VCC #PWR017
U 1 1 5C99A830
P 3850 4100
F 0 "#PWR017" H 3850 3950 50  0001 C CNN
F 1 "VCC" H 3850 4250 50  0000 C CNN
F 2 "" H 3850 4100 50  0001 C CNN
F 3 "" H 3850 4100 50  0001 C CNN
	1    3850 4100
	1    0    0    -1  
$EndComp
$Comp
L power:GNDREF #PWR018
U 1 1 5C99A88B
P 3850 2250
F 0 "#PWR018" H 3850 2000 50  0001 C CNN
F 1 "GNDREF" H 3850 2100 50  0000 C CNN
F 2 "" H 3850 2250 50  0001 C CNN
F 3 "" H 3850 2250 50  0001 C CNN
	1    3850 2250
	1    0    0    -1  
$EndComp
$Comp
L power:GNDREF #PWR019
U 1 1 5C99AB18
P 3850 4200
F 0 "#PWR019" H 3850 3950 50  0001 C CNN
F 1 "GNDREF" H 3850 4050 50  0000 C CNN
F 2 "" H 3850 4200 50  0001 C CNN
F 3 "" H 3850 4200 50  0001 C CNN
	1    3850 4200
	1    0    0    -1  
$EndComp
Wire Wire Line
	3850 4100 3550 4100
Wire Wire Line
	3850 4200 3550 4200
Wire Wire Line
	3850 2150 3600 2150
Wire Wire Line
	3850 2250 3600 2250
Wire Wire Line
	2300 2250 2100 2250
Wire Wire Line
	2100 2250 2100 4200
Wire Wire Line
	2250 4200 2100 4200
Connection ~ 2100 4200
Text GLabel 2600 2450 2    60   Input ~ 0
AHload
Text GLabel 2550 4450 2    60   Input ~ 0
ALload
Wire Wire Line
	2300 2150 2250 2150
Wire Wire Line
	2250 2150 2250 2450
Wire Wire Line
	2250 2450 2600 2450
Wire Wire Line
	2250 4100 2250 4450
Wire Wire Line
	2250 4450 2550 4450
$Comp
L Schematic-rescue:74LS593-74LS593 IC9
U 1 1 5C99FB31
P 6350 1700
F 0 "IC9" H 6350 2400 50  0000 C CNN
F 1 "74LS593" H 6350 2300 50  0000 C CNN
F 2 "Housings_DIP:DIP-20_W7.62mm_Socket" H 6350 1700 50  0001 C CNN
F 3 "" H 6350 1700 50  0001 C CNN
	1    6350 1700
	1    0    0    -1  
$EndComp
$Comp
L Schematic-rescue:74LS593-74LS593 IC10
U 1 1 5C99FFFA
P 6350 3650
F 0 "IC10" H 6350 4350 50  0000 C CNN
F 1 "74LS593" H 6350 4250 50  0000 C CNN
F 2 "Housings_DIP:DIP-20_W7.62mm_Socket" H 6350 3650 50  0001 C CNN
F 3 "" H 6350 3650 50  0001 C CNN
	1    6350 3650
	1    0    0    -1  
$EndComp
Entry Wire Line
	4000 1150 4100 1250
Entry Wire Line
	4000 1250 4100 1350
Entry Wire Line
	4000 1350 4100 1450
Entry Wire Line
	4000 1450 4100 1550
Entry Wire Line
	4000 1550 4100 1650
Entry Wire Line
	4000 1650 4100 1750
Entry Wire Line
	4000 1750 4100 1850
Entry Wire Line
	4000 1850 4100 1950
Entry Wire Line
	4000 3100 4100 3200
Entry Wire Line
	4000 3200 4100 3300
Entry Wire Line
	4000 3300 4100 3400
Entry Wire Line
	4000 3400 4100 3500
Entry Wire Line
	4000 3500 4100 3600
Entry Wire Line
	4000 3600 4100 3700
Entry Wire Line
	4000 3700 4100 3800
Wire Wire Line
	4100 3200 5650 3200
Wire Wire Line
	4100 3300 5650 3300
Wire Wire Line
	4100 3400 5650 3400
Wire Wire Line
	4100 3500 5650 3500
Wire Wire Line
	4100 3600 5650 3600
Wire Wire Line
	4100 3700 5650 3700
Wire Wire Line
	4100 3800 5650 3800
Wire Bus Line
	4000 650  3350 650 
Wire Wire Line
	4100 1250 5650 1250
Wire Wire Line
	5650 1350 4100 1350
Wire Wire Line
	4100 1450 5650 1450
Wire Wire Line
	5650 1550 4100 1550
Wire Wire Line
	4100 1650 5650 1650
Wire Wire Line
	5650 1750 4100 1750
Wire Wire Line
	4100 1850 5650 1850
Wire Wire Line
	5650 1950 4100 1950
Wire Wire Line
	7000 3200 7050 3200
Wire Wire Line
	4950 4100 5350 4100
Entry Wire Line
	4000 3800 4100 3900
Wire Wire Line
	5650 3900 4100 3900
$Comp
L power:VCC #PWR020
U 1 1 5C9A1A2E
P 8550 1300
F 0 "#PWR020" H 8550 1150 50  0001 C CNN
F 1 "VCC" H 8550 1450 50  0000 C CNN
F 2 "" H 8550 1300 50  0001 C CNN
F 3 "" H 8550 1300 50  0001 C CNN
	1    8550 1300
	1    0    0    -1  
$EndComp
Wire Wire Line
	6950 1550 8550 1550
NoConn ~ 7000 1250
$Comp
L power:GNDREF #PWR021
U 1 1 5C9A1C76
P 8550 3650
F 0 "#PWR021" H 8550 3400 50  0001 C CNN
F 1 "GNDREF" H 8550 3500 50  0000 C CNN
F 2 "" H 8550 3650 50  0001 C CNN
F 3 "" H 8550 3650 50  0001 C CNN
	1    8550 3650
	1    0    0    -1  
$EndComp
$Comp
L power:GNDREF #PWR022
U 1 1 5C9A1E6F
P 8550 1700
F 0 "#PWR022" H 8550 1450 50  0001 C CNN
F 1 "GNDREF" H 8550 1550 50  0000 C CNN
F 2 "" H 8550 1700 50  0001 C CNN
F 3 "" H 8550 1700 50  0001 C CNN
	1    8550 1700
	1    0    0    -1  
$EndComp
Text GLabel 7950 3900 2    60   Input ~ 0
~ARena
$Comp
L power:VCC #PWR023
U 1 1 5C9A2F13
P 8550 3250
F 0 "#PWR023" H 8550 3100 50  0001 C CNN
F 1 "VCC" H 8550 3400 50  0000 C CNN
F 2 "" H 8550 3250 50  0001 C CNN
F 3 "" H 8550 3250 50  0001 C CNN
	1    8550 3250
	1    0    0    -1  
$EndComp
Text GLabel 2550 5400 2    60   Input ~ 0
~ARena
Wire Wire Line
	2100 5400 2550 5400
Text Label 4150 1250 0    60   ~ 0
adr15
Text Label 4150 1350 0    60   ~ 0
adr14
Text Label 4150 1450 0    60   ~ 0
adr13
Text Label 4150 1550 0    60   ~ 0
adr12
Text Label 4150 1650 0    60   ~ 0
adr11
Text Label 4150 1750 0    60   ~ 0
adr10
Text Label 4150 1850 0    60   ~ 0
adr9
Text Label 4150 1950 0    60   ~ 0
adr8
Text Label 4150 3200 0    60   ~ 0
adr7
Text Label 4150 3300 0    60   ~ 0
adr6
Text Label 4150 3400 0    60   ~ 0
adr5
Text Label 4150 3500 0    60   ~ 0
adr4
Text Label 4150 3600 0    60   ~ 0
adr3
Text Label 4150 3700 0    60   ~ 0
adr2
Text Label 4150 3800 0    60   ~ 0
adr1
Text Label 4150 3900 0    60   ~ 0
adr0
Text Notes 6250 1600 0    60   ~ 0
PChi
Text Notes 6250 3550 0    60   ~ 0
PClo
Text GLabel 1200 4900 0    60   Input ~ 0
d[0..7]
Text GLabel 3350 650  0    60   Input ~ 0
adr[0..15]
Text Notes 1400 4850 0    60   ~ 0
DataBus
Text Notes 3450 600  0    60   ~ 0
AddressBus
Text GLabel 7800 1350 2    60   Input ~ 0
~Reset
Wire Wire Line
	6950 5900 6950 5850
Wire Wire Line
	6750 5900 6950 5900
Wire Wire Line
	6750 6000 6950 6000
Wire Wire Line
	6950 6000 6950 6050
Wire Wire Line
	5100 4750 5450 4750
Text GLabel 5100 4850 0    60   Input ~ 0
Carry
Text GLabel 5100 4950 0    60   Input ~ 0
Overflow
Text GLabel 5100 5050 0    60   Input ~ 0
Zero
Text GLabel 5100 5150 0    60   Input ~ 0
Negative
Text GLabel 5100 5250 0    60   Input ~ 0
DivByZero
Wire Wire Line
	5100 4850 5450 4850
Wire Wire Line
	5100 4950 5450 4950
Wire Wire Line
	5100 5050 5450 5050
Wire Wire Line
	5100 5150 5450 5150
Wire Wire Line
	5100 5250 5450 5250
Text GLabel 4250 5500 0    60   Input ~ 0
JumpOp[0..2]
Wire Bus Line
	4250 5500 4500 5500
Entry Wire Line
	4500 5750 4600 5850
Entry Wire Line
	4500 5650 4600 5750
Entry Wire Line
	4500 5550 4600 5650
Wire Wire Line
	5450 5650 4600 5650
Wire Wire Line
	4600 5750 5450 5750
Wire Wire Line
	5450 5850 4600 5850
Text Label 5000 5650 0    60   ~ 0
JumpOp0
Text Label 5000 5750 0    60   ~ 0
JumpOp1
Text Label 5000 5850 0    60   ~ 0
JumpOp2
Text GLabel 5100 5350 0    60   Input ~ 0
~TXready
Text GLabel 5100 5450 0    60   Input ~ 0
~RXready
Wire Wire Line
	5100 5350 5450 5350
Wire Wire Line
	5100 5450 5450 5450
Wire Wire Line
	8550 1550 8550 1300
Wire Wire Line
	6950 1650 8550 1650
Text GLabel 7250 1450 2    60   Input ~ 0
Lo
Wire Wire Line
	8550 1650 8550 1700
Wire Wire Line
	6950 3500 8550 3500
Wire Wire Line
	8550 3500 8550 3250
Wire Wire Line
	6950 3600 8550 3600
Wire Wire Line
	8550 3600 8550 3650
Text GLabel 7300 3400 2    60   Input ~ 0
Lo
Wire Wire Line
	5250 6050 5450 6050
Text Notes 1550 7350 0    60   ~ 0
The address bus value, which goes to the RAM & ROM,\ncomes either from the Address register (ARhi, ARlo)\nor the Program Counter (PChi, PClo). The ARena line\nenables one and disables the other.\n\nThe address register loads from the databus, using\nthe AHload and ALload control lines.\n\nThe PC can keep its current value, increment on\nPCincr or load when the /Z line (pin 6) from the\nJump logic goes low. On a jump, the PC loads from\nthe Address register.\n\nThere are eight jump options: never (Lo), five\nALU flag results and two UART status results.\nThe JumpOp control lines choose how to jump.
Text GLabel 5100 4750 0    60   Input ~ 0
Lo
Text GLabel 8000 1950 2    60   Input ~ 0
~ARena
Wire Wire Line
	7050 3900 7950 3900
Wire Wire Line
	7050 1950 8000 1950
Text GLabel 7000 5000 2    60   Input ~ 0
~PCload
Wire Wire Line
	6850 5000 7000 5000
Text GLabel 7300 4000 2    60   Input ~ 0
~PCload
Wire Wire Line
	5350 4000 5650 4000
Text GLabel 7350 2050 2    60   Input ~ 0
~PCload
Wire Wire Line
	7000 2050 7350 2050
Wire Wire Line
	7000 4000 7300 4000
Wire Wire Line
	7000 3400 7100 3400
Wire Wire Line
	7000 1450 7150 1450
Wire Wire Line
	7000 1350 7800 1350
Text GLabel 7950 3300 2    60   Input ~ 0
~Reset
Wire Wire Line
	7000 3300 7950 3300
Wire Wire Line
	7050 3800 7300 3800
Wire Wire Line
	7000 4100 7100 4100
Wire Wire Line
	7050 1850 7150 1850
Text GLabel 4950 4100 0    60   Input ~ 0
~Clkbar
Text GLabel 7300 3800 2    60   Input ~ 0
PCincr
Text GLabel 5200 2050 0    60   Input ~ 0
~Clkbar
NoConn ~ 6850 4900
Wire Wire Line
	5200 2050 5650 2050
Text GLabel 7100 4100 2    60   Input ~ 0
Hi
Wire Wire Line
	5350 4000 5350 4100
Connection ~ 5350 4100
Wire Wire Line
	7000 3700 7100 3700
Wire Wire Line
	7100 3700 7100 3400
Connection ~ 7100 3400
Wire Wire Line
	7150 1850 7150 1750
Connection ~ 7150 1450
Wire Wire Line
	7000 1750 7150 1750
Connection ~ 7150 1750
Wire Wire Line
	2100 4200 2100 5400
Wire Wire Line
	5350 4100 5650 4100
Wire Wire Line
	7100 3400 7300 3400
Wire Wire Line
	7150 1450 7250 1450
Wire Wire Line
	7150 1750 7150 1450
Wire Wire Line
	7150 2150 7150 1850
Wire Wire Line
	7000 2150 7150 2150
Connection ~ 7150 1850
Wire Wire Line
	5650 2150 5650 2550
Wire Wire Line
	5650 2550 7050 2550
Wire Wire Line
	7050 2550 7050 3200
$Comp
L Schematic-rescue:74LS251-RESCUE-Schematic-Schematic-rescue IC11
U 1 1 5CDB88DB
P 6150 5400
F 0 "IC11" H 6150 6275 50  0000 C CNN
F 1 "74HCT151" H 6150 6184 50  0000 C CNN
F 2 "Package_DIP:DIP-16_W7.62mm_Socket" H 6150 5400 50  0001 C CNN
F 3 "" H 6150 5400 50  0001 C CNN
	1    6150 5400
	1    0    0    -1  
$EndComp
Text GLabel 5250 6050 0    60   Input ~ 0
Clk
Wire Bus Line
	4500 5500 4500 5750
Wire Bus Line
	4000 650  4000 3800
Wire Bus Line
	1950 1350 1950 4900
$EndSCHEMATC
