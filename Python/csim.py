...
pg.init()
screen = pg.display.set_mode((160, 120), pg.SCALED)
framebuffer = pg.surfarray.array3d(screen)
clock = pg.time.Clock()


def disp():
    pg.surfarray.blit_array(screen, framebuffer)
    pg.display.update()


@njit(fastmath=True)
def plot(framebuffer, x, y, colour):
    r = ((colour >> 4) & 0x3) << 6
    g = ((colour >> 2) & 0x3) << 6
    b = ((colour >> 0) & 0x3) << 6
...
def run():
    ...
    file = open('../27Cucode.rom', 'rb')
    DecodeRom = list(file.read())
    file.close()

    file = open('../Examples/mandelbrot.bin', 'rb')
    Ram = list(file.read())
